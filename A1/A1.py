"""
STUDENT NAME: Joey LITALIEN
STUDENT ID: 260532617
...
def drawScene():
    ...
    # Upper cube
    glPushMatrix()
    glTranslatef(-1.4, 0.875, -5.0)
    glRotatef(-50, 0, 1, 0) 
    glScalef(0.50, 0.75, 0.50)
    drawColoredCube(1)
    glPopMatrix()
    # Sphere
    glPushMatrix()
    glColor4f(0.0, 0.6, 1.0, 1);
    glTranslatef(1.4, 0.25, -4.0)
    glScalef(1.5, 1.5, 1.5)
    gluSphere(myQuadric, 0.5, 20, 20)
    glPopMatrix()
    # Tea pot
    glPushMatrix()
    glColor4f(1.0, 1.0, 0.0, 1);
    glTranslatef(0.3, -0.12, -2.8)
    ...
    # Bigger torus
    glPushMatrix()
    glColor4f(0.15, 0.73, 0.27, 1);
    glTranslatef(-0.20, -0.46, -1.5)
    glRotatef(90.0, 1, 0, 0) 
    glScalef(0.5, 0.5, 0.5)
    glutSolidTorus(0.05, 0.6, 10, 20)
    glPopMatrix()
    # Smaller torus
    glPushMatrix()
    glColor4f(1, 1, 1, 1);
...
def Viewport3():
    ...
    # Draw view volume, invariant under rotation/translation
    drawViewVolume(left, right, bottom, top, near, far)
    # Allow rotation and translation for the scene
    glTranslatef(-eyeX, eyeY, -eyeZ)
    glRotatef(pan, 0, 1, 0)
    glRotatef(-tilt, 1, 0, 0)
    # Draw scene and moving camera
    drawScene()
    drawMovingCamera()
    glPopMatrix()
    #  ---------------   END SOLUTION  -----------------------
...
def perspective(fovy, aspect, near, far):
    ...
    P = np.eye(4)
    #  ---------------   BEGIN SOLUTION  -----------------------
    P = projective(near, far)
    P = translate([0, 0, near]) * P
    P = scale([1.0/(near*aspect*tan(deg2rad(fovy/2))), 1.0/(near*tan(deg2rad(fovy/2))), -2.0/(near-far)]) * P
    P = translate([0.0, 0.0, 1.0]) * P
    P = scale([1.0, 1.0, -1.0]) * P
    #  ---------------   END SOLUTION  -----------------------
    return np.matrix(P)
...
def Viewport4():
    ...
    if(viewport4Sol1):
        ...
        glMultMatrixd(transpose(perspective(60, 1, .01, 20)))

        glMatrixMode(GL_MODELVIEW)
        glLoadIdentity()
        # Rotate transform
        glMultMatrixd(transpose(rotateX(-rad2deg(arctan2(movingCameraPos[1], movingCameraRadius)))))
        glMultMatrixd(transpose(rotateY(movingCameraAngle)))
        # Translate transform
        glMultMatrixd(transpose(translate([-movingCameraPos[0], -movingCameraPos[1], -movingCameraPos[2]])))
        # Draw scene
        drawScene()

        glPushMatrix()
        # Translate view volume
        glMultMatrixd(transpose(translate([eyeX, eyeY, eyeZ])))
        # Rotate view volume
        glMultMatrixd(transpose(rotateY(pan)))
        glMultMatrixd(transpose(rotateX(-tilt)))
        # Draw view volume
        drawViewVolume(left, right, bottom, top, near, far)
        glPopMatrix()
        #  ---------------   END SOLUTION  -----------------------
